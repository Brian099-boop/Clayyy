...
const Shop = () => {
  ...
  return (
    <div className="container mx-auto py-10">
      <h2 className="text-4xl text-center font-bold mb-10 text-teal-700">Users</h2>

      {/* User Grid (6 boxes in a row) */}
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
        {users.map((user) => {
          ...
          return (
            <div
              key={user.id}
              className="relative group bg-teal-800 rounded-2xl overflow-hidden shadow-xl transition duration-300 ease-in-out transform hover:scale-105 hover:shadow-2xl"
            >
              <div className="pl-6 pr-6 pt-6 pb-4">
                ...
                <p className="text-lg font-bold text-teal-400 mt-4">Company: {user.company.name}</p>
                    ...
                    {productReviews.length > 0 ? (
                      productReviews.map((review) => (
                        <li key={review.id} className="mt-2">
                          <p>"{review.body}"</p>
                          <p className="text-sm text-teal-200">By: {review.name}</p>
                        </li>
                      ))
                    ) : (
                      <p>No reviews yet.</p>
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-teal-600 to-transparent opacity-0 group-hover:opacity-70 transition duration-300 ease-in-out"></div>
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out">
                <span className="text-white text-lg font-semibold px-6 py-2 rounded-lg bg-black bg-opacity-50 transform scale-90 group-hover:scale-100 transition duration-300 ease-in-out">
                  View Profile
                </span>
              </div>
            </div>
